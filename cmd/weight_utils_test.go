...
func TestParseWeightFlag_Valid(t *testing.T) {
	tests := []struct {
		...
		{"integer weight with =", "scenario-1=2", "scenario-1", 2.0},
		{"decimal weight with =", "scenario-2=2.5", "scenario-2", 2.5},
		{"leading decimal with =", "scenario-3=.5", "scenario-3", 0.5},
		{"with hyphens and =", "pod-scenarios=3.0", "pod-scenarios", 3.0},
		{"with underscores and =", "node_scenarios=1.5", "node_scenarios", 1.5},
		{"mixed naming with =", "test-scenario_1=4.2", "test-scenario_1", 4.2},
		{"integer weight with :", "scenario-1:2", "scenario-1", 2.0},
		{"decimal weight with :", "scenario-2:2.5", "scenario-2", 2.5},
		{"with hyphens and :", "pod-scenarios:3.0", "pod-scenarios", 3.0},
	}
...
func TestParseWeightFlag_InvalidFormat(t *testing.T) {
	tests := []struct {
		...
		{"missing equals or colon", "scenario1", "invalid --weight format"},
		{"missing weight value", "scenario-1=", "invalid --weight format"},
		{"missing scenario name", "=2.0", "invalid --weight format"},
		{"missing scenario with colon", ":2.0", "invalid --weight format"},
		{"empty both sides", "=", "invalid --weight format"},
		{"only separator", ":", "invalid --weight format"},
		{"invalid weight chars", "scenario-1=2.0a", "invalid weight value"},
	}
...
func TestParseWeightFlag_InvalidWeight(t *testing.T) {
	tests := []struct {
		...
		{"negative weight", "scenario-1=-1.5", "weight must be greater than 0"},
		{"very small weight", "scenario-1=0.0", "weight must be greater than 0"},
		{"zero weight with colon", "scenario-1:0", "weight must be greater than 0"},
		{"negative weight with colon", "scenario-1:-1.5", "weight must be greater than 0"},
	}
...
func TestParseAndApplyWeightOverrides_UsesScenarioName(t *testing.T) {
	scenarios := models.ScenarioSet{
		"node-a7f3": models.ScenarioNode{
			Scenario: models.Scenario{Name: "pod-scenarios", ResiliencyWeight: 1.0},
		},
		"node-b2c1": models.ScenarioNode{
			Scenario: models.Scenario{Name: "node-scenarios", ResiliencyWeight: 1.0},
		},
		"_comment": models.ScenarioNode{
			Scenario: models.Scenario{Comment: "generated scenario set"},
		},
	}

	err := ParseAndApplyWeightOverrides(scenarios, []string{"pod-scenarios=2.0"})
	if err != nil {
		t.Fatalf("ParseAndApplyWeightOverrides() unexpected error: %v", err)
	}

	if scenarios["node-a7f3"].ResiliencyWeight != 2.0 {
		t.Errorf("scenario weight = %v, want 2.0", scenarios["node-a7f3"].ResiliencyWeight)
	}
}
...
func TestParseAndApplyWeightOverrides_InvalidFlag(t *testing.T) {
	...
	tests := []struct {
		...
		{"invalid weight", []string{"scenario-1=abc"}, "invalid weight value"},
	}
...
func TestGetScenarioNames(t *testing.T) {
	scenarios := models.ScenarioSet{
		"node-1":   models.ScenarioNode{Scenario: models.Scenario{Name: "scenario-1"}},
		"node-2":   models.ScenarioNode{Scenario: models.Scenario{Name: "scenario-2"}},
		"node-3":   models.ScenarioNode{Scenario: models.Scenario{Name: "scenario-3"}},
		"_comment": models.ScenarioNode{Scenario: models.Scenario{Comment: "metadata"}},
	}

	names := getScenarioNames(scenarios)

	// Check that all scenario names are included
	for _, name := range []string{"scenario-1", "scenario-2", "scenario-3"} {
		if !strings.Contains(names, name) {
			t.Errorf("getScenarioNames() = %v, missing scenario %v", names, name)
		}
	}
	if strings.Contains(names, "node-") || strings.Contains(names, "metadata") {
		t.Errorf("getScenarioNames() = %v, contains a node ID or comment", names)
	}
