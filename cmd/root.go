...
func Execute(providerFactory *factory.ProviderFactory, scenarioOrchestrator *scenarioorchestrator.ScenarioOrchestrator, config config.Config) {
	...
	graphScaffoldCmd.Flags().StringArray("weight", []string{}, "set resiliency weights for scenarios by name (format: scenario-name=weight or scenario-name:weight, can be repeated)")
	...
	randomScaffoldCmd.Flags().StringArray("weight", []string{}, "set resiliency weights for scenarios by name (format: scenario-name=weight or scenario-name:weight, can be repeated)")
	...
	dashboardCmd := NewDashboardCommand(scenarioOrchestrator, config)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to fetch krknctl version: %v\n", err)
		fmt.Println(err)
