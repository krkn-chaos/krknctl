...
Optional --chaos-assets-dir and --database-dir default to the stock image paths. Operators configure host paths; in-container paths match the image unless overridden.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// On Darwin the volume source must be the VM-internal socket path so that
			// podman create bind-mounts within the VM rather than trying to statfs a
			// macOS path that the VM kernel rejects.
			podmanSockSrc := hostSock
			if containerRuntime == orcmodels.Podman && runtime.GOOS == "darwin" {
				machine, err := podmanRunningMachineName()
				if err != nil {
					return fmt.Errorf("could not find running podman machine: %w", err)
				}
				vmSock, err := podmanRunningMachineSocketPath(machine)
				if err != nil {
					return fmt.Errorf("could not find podman socket in VM: %w", err)
				}
				podmanSockSrc = vmSock
			}
			switch containerRuntime {
			case orcmodels.Podman:
				volumes[podmanSockSrc] = "/run/podman/podman.sock"
