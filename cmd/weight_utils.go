...
func ParseAndApplyWeightOverrides(scenarios map[string]models.ScenarioNode, weightFlags []string) error {
	...
	// Build set of valid scenario names (excluding _comment metadata). The map
	// key is a graph node ID and may be randomized independently of the name.
	validNames := make(map[string]bool)
	for nodeID, scenario := range scenarios {
		if nodeID != "_comment" {
			validNames[scenario.Name] = true
		}
	}
	...
	for scenarioName, weight := range weights {
		if !validNames[scenarioName] {
			return fmt.Errorf("unknown scenario %q in --weight flag. Available scenarios: %s",
				scenarioName, getScenarioNames(scenarios))
		}

		// Update the scenario identified by its user-facing name, not its graph
		// node ID.
		for nodeID, scenario := range scenarios {
			if nodeID != "_comment" && scenario.Name == scenarioName {
				scenario.ResiliencyWeight = weight
				scenarios[nodeID] = scenario
			}
		}
	}
...
// parseWeightFlag parses a single weight flag in format "scenario-name=weight" or "scenario-name:weight"
...
func parseWeightFlag(flag string) (string, float64, error) {
	...
	if strings.Contains(flag, "=") {
		...
		return "", 0, fmt.Errorf("invalid --weight format %q. Expected format: ScenarioName=weight or ScenarioName:weight (e.g., critical-service=2 or critical-service=2.5)", flag)
	}
