...
func NewVisualizeCommand(scenarioOrchestrator *scenarioorchestrator.ScenarioOrchestrator, config config.Config) *cobra.Command {
	var command = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !deleteFlag && grafanaPassword == "" {
				return fmt.Errorf("required flag \"grafana-password\" not set")
			}
			...
			if grafanaPassword != "" {
				...
				displayEnv["GRAFANA_PASSWORD"] = ParsedField{value: grafanaPassword, secret: true}
			}
			if deleteFlag {
			...
			_, err = (*scenarioOrchestrator).RunAttached(visualizeImage, containerName, environment, false, volumes, os.Stdout, os.Stderr, &commChan, conn, nil, nil, nil, false)
			if err != nil {
				return err
			}
	...
	command.Flags().String("grafana-password", "", "Grafana admin password (required unless --delete is set)")
