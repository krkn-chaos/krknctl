...
// Pin runtime-spec to v1.2.1. v1.3.0 changed LinuxPids.Limit from int64 to
// *int64, which is incompatible with podman v5.8.2 (and every released podman
// v5.x, plus the runtime-tools it requires), all of which use the int64 field.
// v1.3.0 is only pulled transitively by go.podman.io/storage, which does not
// use LinuxPids.Limit, so pinning back to v1.2.1 keeps all vendored code
// pristine and building without hand-patching the vendor tree. Revisit once
// podman itself moves to runtime-spec v1.3.0.
