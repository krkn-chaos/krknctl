...
func CommonTestScenarioOrchestratorRun(t *testing.T, so scenarioorchestrator.ScenarioOrchestrator, conf krknctlconfig.Config, duration int) string {
	...
	containerID, err := so.Run(registryURI+":"+scenario.Name, containerName, env, false, map[string]string{}, nil, ctx, nil, nil)
	assert.Nil(t, err)
	...
	containerID, err = so.Run(pr.GetPrivateRegistryURI()+":"+scenario.Name, containerName, env, false, map[string]string{}, nil, ctx, &pr, nil)
...
func CommonTestScenarioOrchestratorListRunningScenarios(t *testing.T, so scenarioorchestrator.ScenarioOrchestrator, conf krknctlconfig.Config) {
	...
	_, err = so.Run(registryURI+":"+scenario.Name, containerName2, env, false, map[string]string{}, nil, ctx, nil, nil)
	assert.Nil(t, err)
	_, err = so.Run(registryURI+":"+scenario.Name, containerName1, env, false, map[string]string{}, nil, ctx, nil, nil)
	assert.Nil(t, err)
