...
func findRepoRootKubeconfigFixture(t *testing.T) (string, bool) {
	...
	for range 12 {
		candidate := filepath.Join(dir, "tests", "data", "kubeconfig")
...
// prepareTestKubeconfigForOrchestrator uses the default kubeconfig when present, otherwise the repo fixture tests/data/kubeconfig.
func prepareTestKubeconfigForOrchestrator(t *testing.T, conf krknctlconfig.Config) *string {
	t.Helper()
	kc, err := utils.PrepareKubeconfig(nil, conf)
	assert.Nil(t, err)
	if kc != nil {
		return kc
	}
	path, ok := findRepoRootKubeconfigFixture(t)
	if !ok {
		t.Skip("no kubeconfig at default path and tests/data/kubeconfig not found (run tests from krknctl repo checkout)")
	}
	kc, err = utils.PrepareKubeconfig(&path, conf)
	assert.Nil(t, err)
	assert.NotNil(t, kc)
	return kc
}
...
func CommonTestScenarioOrchestratorRun(t *testing.T, so scenarioorchestrator.ScenarioOrchestrator, conf krknctlconfig.Config, duration int) string {
	...
	kubeconfig := prepareTestKubeconfigForOrchestrator(t, conf)
	...
	containerID, err := so.Run(registryURI+":"+scenario.Name, containerName, env, false, map[string]string{}, nil, ctx, nil, nil, nil, false)
	assert.Nil(t, err)
	...
	if os.Getenv("CI_FORK_PR") == "false" {
		...
		containerID, err = so.Run(pr.GetPrivateRegistryURI()+":"+scenario.Name, containerName, env, false, map[string]string{}, nil, ctx, &pr, nil, nil, false)
...
func CommonTestScenarioOrchestratorRunAttached(t *testing.T, so scenarioorchestrator.ScenarioOrchestrator, conf krknctlconfig.Config, duration int) string {
	...
	containerID, err := so.RunAttached(registryURI+":"+scenario.Name, containerName1, env, false, map[string]string{}, os.Stdout, os.Stderr, nil, ctx, nil, nil, nil, false)
	if err != nil {
	...
	containerID, err = so.RunAttached(registryURI+":"+scenario.Name, containerName2, env, false, map[string]string{}, os.Stdout, os.Stderr, nil, ctx, nil, nil, nil, false)
	if err != nil {
		fmt.Println("ERROR -> " + err.Error())
	}
	assert.NotNil(t, err)
...
`
	...
	assert.Nil(t, err)
	assert.NotNil(t, scenario)
	kubeconfig := prepareTestKubeconfigForOrchestrator(t, config)
	fmt.Println("KUBECONFIG PARSED -> " + *kubeconfig)
	...
	go func() {
		so.RunGraph(nodes, executionPlan, map[string]string{}, map[string]string{}, false, commChannel, nil, uid, false)
	}()
...
func CommonTestScenarioOrchestratorResolveContainerName(t *testing.T, so scenarioorchestrator.ScenarioOrchestrator, conf krknctlconfig.Config, duration int) {
	...
	containerID, err := so.RunAttached(registryURI+":"+scenario.Name, containerName, env, false, map[string]string{}, os.Stdout, os.Stderr, nil, ctx, nil, nil, nil, false)
	assert.Nil(t, err)
	assert.NotNil(t, containerID)
...
func CommonTestScenarioOrchestratorListRunningScenarios(t *testing.T, so scenarioorchestrator.ScenarioOrchestrator, conf krknctlconfig.Config) {
	...
	_, err = so.Run(registryURI+":"+scenario.Name, containerName2, env, false, map[string]string{}, nil, ctx, nil, nil, nil, false)
	assert.Nil(t, err)
	_, err = so.Run(registryURI+":"+scenario.Name, containerName1, env, false, map[string]string{}, nil, ctx, nil, nil, nil, false)
	assert.Nil(t, err)
