// Package assist provides simple platform-based GPU acceleration detection
...
// GetAssistImageURI returns the appropriate assist container image for detected GPU type
func (d *PlatformGPUDetector) GetAssistImageURI(gpuType GPUAcceleration) (string, error) {
	switch gpuType {
	case GPUAccelerationAppleSilicon:
		return d.config.GetAssistImageURI("apple-silicon")
	case GPUAccelerationNVIDIA:
		return d.config.GetAssistImageURI("nvidia")
	case GPUAccelerationGeneric:
		return d.config.GetAssistImageURI("generic")
	default:
...
// AutoSelectAssistConfig automatically selects the best assist configuration
func (d *PlatformGPUDetector) AutoSelectAssistConfig(ctx context.Context, noGPU bool) (string, GPUAcceleration, map[string]string, error) {
	// Detect GPU acceleration type
	gpuType := d.DetectGPUAcceleration(ctx, noGPU)

	// Get appropriate container image
	imageURI, err := d.GetAssistImageURI(gpuType)
	if err != nil {
		return "", "", nil, fmt.Errorf("failed to get assist image for %s: %w", gpuType, err)
	}
