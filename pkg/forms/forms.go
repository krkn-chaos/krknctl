...
func (f *Form) Run() (*FormResult, error) {
	...
	if len(f.RequiredItems) > 0 {
		fmt.Printf("\n%s\n", color.New(color.FgGreen, color.Bold).Sprint("🔴 Required Fields"))
	...
	if len(f.OptionalItems) > 0 {
		...
		if _, err := askOptional.Run(); err == nil {
			fmt.Printf("\n%s\n", color.New(color.FgYellow, color.Bold).Sprint("🟡 Optional Fields"))
	...
	if len(f.GlobalItems) > 0 {
		...
		if _, err := askGlobal.Run(); err == nil {
			fmt.Printf("\n%s\n", color.New(color.FgBlue, color.Bold).Sprint("🌐 Global Values"))
...
func (f *Form) processItem(item FormPromptItem, result *FormResult) error {
	...
	for {
		...
		if err != nil {
			...
			fmt.Printf("%s %v\n", color.New(color.FgRed).Sprint("❌ Validation error:"), err)
			continue
		}
...
func (r *FormResult) PrintSummary(fields []typing.InputField) {
	...
	fmt.Printf("\n%s\n", color.New(color.FgCyan, color.Bold, color.Underline).Sprint("📋 Form Summary"))
